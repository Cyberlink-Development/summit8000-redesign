                                            ...
                                            <tr class="id{{$row->id}}">
                                                ...
                                                <td class="text-left">
                                                    ...
                                                    @if(!is_empty_posttype($row->id))
                                                        |
                                                        <a href="#{{$row->id}}" class="btn-delete" data-has-seo="{{ $row->seo ? 1 : 0 }}">Delete</a>
                                                    @endif
                                                </td>
                                            </tr>
                                        @endforeach
    ...
    <script type="text/javascript">
        jQuery(document).ready(function () {
            $('.btn-delete').on('click', function (e) {
                e.preventDefault();
                var hasSeo = $(this).data('has-seo');

                var message = hasSeo == 1
                    ? 'SEO data will also be deleted. Are you sure?'
